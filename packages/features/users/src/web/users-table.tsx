...
export function UsersTable({ principal, client = new UsersClient(), onCreate, onSelect, refreshKey = 0 }: { principal: Pick<AuthenticatedPrincipal, "permissions">; client?: UsersTableClient; onCreate?: () => void; onSelect?: (user: UserDto) => void; refreshKey?: number }) {
  ...
  const canCreate = principal.permissions.includes("users:create");
  ...
  return <section className="effect-admin-card" aria-label="فهرست کاربران">
    ...
    <div className="effect-table-wrap"><table className="effect-table"><thead><tr><th>کاربر</th><th>شماره موبایل</th><th>وضعیت</th><th>آخرین ورود</th><th><span className="effect-sr-only">عملیات</span></th></tr></thead><tbody>{page.items.map((user) => <tr key={user.id}><td><strong>{user.firstName} {user.lastName}</strong></td><td dir="ltr">{user.phone}</td><td><span className={`effect-badge effect-badge--${user.status.toLowerCase()}`}>{user.status === "SUSPENDED" ? "تعلیق‌شده" : "فعال"}</span></td><td>{user.lastLoginAt ? new Intl.DateTimeFormat("fa-IR", { dateStyle: "medium" }).format(new Date(user.lastLoginAt)) : "—"}</td><td><Button variant="ghost" onClick={() => onSelect?.(user)} aria-label={`جزئیات ${user.firstName} ${user.lastName}`}>جزئیات</Button></td></tr>)}</tbody></table></div>
