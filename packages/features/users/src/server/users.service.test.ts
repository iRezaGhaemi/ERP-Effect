...
function createManager(): EntityManager {
  ...
  const repository = {
    create(values: Pick<UserEntity, 'phone' | 'firstName' | 'lastName' | 'status'>): UserEntity {
    ...
    async findOneBy(criteria: Partial<Pick<UserEntity, 'phone' | 'status'>>): Promise<UserEntity | null> {
      return (
        users.find(
          (user) =>
...
describe('users service', () => {
  ...
  it('enforces unique normalized phone numbers', async () => {
    ...
    const service = new UsersService({ manager } as never);

    await service.create({ phone: '09121234567', firstName: 'رضا', lastName: 'قایمی' }, actorId);

    await expect(
      service.create({ phone: '+989121234567', firstName: 'رضا', lastName: 'دوم' }, actorId),
